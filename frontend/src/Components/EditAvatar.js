...
const EditAvatar = ({onClose}) => {
  ...
  return (
    <div className='bg-white shadow-2xl rounded-xl absolute ml-[1650px] mt-96 h-[700px] w-[600px]  transform -translate-x-1/2 -translate-y-1/2 p-4 '>
        ...
        <span className='text-2xl font-mono mr-56 from-neutral-800 '>Add Profile Picture</span>
        <span className='ml=10 text-3xl cursor-pointer' onClick={onClose}>X</span>
        </div>
        <hr className='border-2'/>
        
        <img className='h-[500px] w-[520px] m-6 rounded-lg' src={selectedFile ? URL.createObjectURL(selectedFile) : "https://cdn-icons-png.freepik.com/512/10302/10302971.png"} alt="cover" />
        <hr className='border-2'/>
